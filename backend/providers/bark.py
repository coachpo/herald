"""Bark provider - async HTTP implementation with httpx."""
...
def build_bark_payload(
    *,
    ...
    """Build Bark payload from channel config, template, and message."""
    ...
    if not payload.get("body") and message.get("body"):
        payload["body"] = message["body"]
    if not payload.get("title") and message.get("title"):
        payload["title"] = message["title"]

    if channel_config.get("device_key") is not None:
        payload["device_key"] = channel_config.get("device_key")
    if channel_config.get("device_keys") is not None:
        payload["device_keys"] = channel_config.get("device_keys")
...
async def send_bark_push(
    *,
    ...
    """Send Bark push notification via async HTTP."""
    assert_ssrf_safe(server_base_url, block_private_networks=block_private_networks)
    ...
    # Legacy GET fallback for 404/405
    if resp.status_code in {404, 405}:
        ...
        if device_key and body:
            ...
            assert_ssrf_safe(legacy_url, block_private_networks=block_private_networks)

            async with httpx.AsyncClient() as client:
                legacy_resp = await client.get(legacy_url, timeout=timeout)
